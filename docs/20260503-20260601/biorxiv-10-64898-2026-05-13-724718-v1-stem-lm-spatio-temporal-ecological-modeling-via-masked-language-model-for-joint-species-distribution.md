...
## 速览
**TLDR**：联合物种分布模型对生物多样性预测至关重要，但现有方法难以处理大规模、高维度的时空生态数据。本文提出STEM-LM，将每个地点的物种出现或缺失视为token，整个地点的物种集合与时空协变量组成句子，利用Transformer进行掩码语言建模，学习物种间的共现结构。在北美蝴蝶和全球植物数据集上，STEM-LM在判别排序上优于或持平现有方法，且生成的概率校准更好。通过利用部分物种观测，预测性能显著提升，为生态建模提供了灵活可扩展的新框架。 \
**Motivation**：现有联合物种分布模型难以扩展到大规模、高分辨率时空数据，且缺乏灵活性；掩码语言建模为学习共现结构提供了新思路。 \
**Method**：将每个地点的物种组合视为句子，使用Transformer编码，通过掩码部分物种token并预测来学习联合分布，支持任意条件推理。 \
**Result**：在北美蝴蝶和全球植物数据集上，STEM-LM的判别排序性能优于或持平现有方法，且概率校准更准确，利用部分观测可大幅提升预测。 \
**Conclusion**：STEM-LM提供了一种可扩展、灵活的联合物种分布建模方法，适用于大规模生态数据，支持基于部分观测的条件预测。
